...
::: {#def-linind1}
*linearly dependent* if there exists a set of scalars $\{a_{1},a_{2},\dots,a_{k}\}$ such that
...
::: {#def-linind2}
*linearly independent* if the only set of scalars $\{a_{1},a_{2},\dots,a_{k}\}$ that satisfies the above condition is $a_1,a_2,\dots,a_k=0$.
:::
...
::: {#def-lindep1}
*linearly dependent* if there exists a vector $a\in\mathbb{R}^k$ such that $a\neq0$ and $Xa=0$;
:::

or,

::: {#def-lindep2}
*linearly independent* if the only vector $a\in\mathbb{R}^k$ such that $Xa=0$ is $a=0$.
:::
...
::: {#def-vecspace}
A *vector space*, denoted $\mathcal{V}$, refers to a set of vectors which is closed under finite addition and scalar multiplication.
:::

::: {#def-basis}
A set of $k$ linearly independent vectors, $\{X_1,X_2,\dots,X_k\}$, forms a *basis* for vector space $\mathcal{V}$ if $\forall\;y\in\mathcal{V}$ there exists a set of $k$ scalars such that, $$
...
::: {#def-colspace}
*column space* (or *span*) of $X$, denoted $\mathcal{S}(X)$, as the vector space generate by the $k$ columns of $X$. Formally, $$
...
::: {#def-orthspan}
*orthogonal column space* (or *orthogonal* *span*) of $X$ as, $$
...
::: {#def-rowrank}
*row rank* of $X$ is the maximum number of linearly independent rows: $$
rowrank(X) \leq n
$$
:::
...
::: {#def-colrank}
*column rank* of $X$ is the maximum number of linearly independent columns:
...
::: {#def-singular}
$A$ is *singular* if the $r(A)<n$,
:::

or that,

::: {#def-nonsingular}
$A$ is *non-singular* if the $r(A)=n$.
:::
...
::: {#def-trace}
the *trace* of $A$ is the sum of all diagonal elements: $$
...
::: {#def-possemi}
$A$ is *positive semi-definite* if for any $x\in\mathbb{R}^n,\;x'Ax\geq0$.
:::
...
::: {#def-posdef}
$A$ is *positive definite* if for any $x\in\mathbb{R}^n,\;x'Ax>0$.
:::
